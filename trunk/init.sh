...
###############################################################
nodejs ~/projects/semantiker/semantiker.js $APP/schema/schema.xml $APP/schema/reprisk_inventory_new.xml $APP/schema/reprisk_db_new.xml
...
#reprisk inventory
./xenesis.sh $APP/schema/reprisk_inventory_new.xml /tmp/reprisk_inventory.sql
psql -h $DBHOST -p $DBPORT -P pager=off  -f  /tmp/reprisk_inventory.sql $DBNAME $DBUSER

#reprisk DB
./xenesis.sh $APP/schema/reprisk_db_new.xml /tmp/reprisk_db.sql
...
#bash ~/projects/authentication.sh $DBNAME $DBUSER $DBHOST
